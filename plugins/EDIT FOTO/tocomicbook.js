...
async function handle(sock, messageInfo) {
    ...
    try {
        ...
        // Kirim ke API JadiComicBook
        const apiUrl = `https://api.betabotz.eu.org/api/maker/jadicomicbook?apikey=${config.apikey.key}&url=${encodeURIComponent(uploadedUrl)}`;
        const response = await axios.get(apiUrl, { responseType: "arraybuffer" });

        const resultPath = path.join("tmp", `comicbook_${Date.now()}.jpg`);
        fs.writeFileSync(resultPath, response.data);

        await sock.sendMessage(remoteJid, {
            image: fs.readFileSync(resultPath),
            caption: `_✅ gambar berhasil diubah ke mode comicbook!_`
        }, { quoted: message });

        fs.unlinkSync(mediaPath);
        fs.unlinkSync(resultPath);
    } catch (error) {
        console.error("❌ ERROR plugin tocomicbook:", error);
        await sock.sendMessage(remoteJid, {
