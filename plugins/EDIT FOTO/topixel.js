...
async function handle(sock, messageInfo) {
    ...
    try {
        ...
        if (!fileUrl) throw new Error('Gagal upload ke Uguu dan Catbox');

        const apiUrl = `https://anabot.my.id/api/ai/toPixel?imageUrl=${encodeURIComponent(fileUrl)}&apikey=freeApikey`;
        ...
        const resultPath = path.join('tmp', `pixel_${Date.now()}.png`);
        fs.writeFileSync(resultPath, imageBuffer);

        await sock.sendMessage(remoteJid, {
            image: fs.readFileSync(resultPath),
            caption: `_✅ gambar berhasil diubah ke mode pixel!_`
        }, { quoted: message });

        fs.unlinkSync(mediaPath);
        fs.unlinkSync(resultPath);
    } catch (error) {
        console.error("❌ Error plugin topixel:", error);
        await sock.sendMessage(remoteJid, {
