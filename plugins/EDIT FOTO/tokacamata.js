...
async function handle(sock, messageInfo) {
    ...
    try {
        ...
        const contents = [
            {
                text: 'Edit the uploaded image while maintaining the characters pose and position without any changes. change the character to wear sunglasses, the glasses look realistic like a person wearing sunglasses in general, without removing or replacing any part of the character.'
            },
            {
                inlineData: {
                    mimeType: mime,
        ...
        for (let part of res.candidates?.[0]?.content?.parts || []) {
            if (part.inlineData) {
                ...
                await sock.sendMessage(remoteJid, {
                    image: imageBuffer,
                    caption: '*✅ selesai diedit jadi menggunakan kacamata*'
                }, { quoted: message });
            }
        }
