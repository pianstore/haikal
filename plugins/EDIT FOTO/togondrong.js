...
async function handle(sock, messageInfo) {
    ...
    try {
        ...
        const contents = [
            {
                text: 'Edit the uploaded image while maintaining the characters pose and position without any changes. change the hair only to give it a natural longish look, without removing or replacing any features of the character.'
            },
            {
                inlineData: {
                    mimeType: mime,
        ...
        for (let part of res.candidates?.[0]?.content?.parts || []) {
            if (part.inlineData) {
                ...
                await sock.sendMessage(remoteJid, {
                    image: imageBuffer,
                    caption: '*✅ selesai diedit jadi gondrong*'
                }, { quoted: message });
            }
        }
