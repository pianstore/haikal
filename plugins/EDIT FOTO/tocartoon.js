...
async function handle(sock, messageInfo) {
    ...
    try {
        ...
        // Kirim ke API JadiCartoon
        const apiUrl = `https://api.betabotz.eu.org/api/maker/jadicartoon?apikey=${config.apikey.key}&url=${encodeURIComponent(fileUrl)}`;
        const response = await axios.get(apiUrl, { responseType: 'arraybuffer' });

        const resultPath = path.join('tmp', `cartoon_${Date.now()}.jpg`);
        fs.writeFileSync(resultPath, response.data);

        await sock.sendMessage(remoteJid, {
            image: fs.readFileSync(resultPath),
            caption: `_✅ gambar berhasil diubah ke mode cartoon!_`
        }, { quoted: message });

        fs.unlinkSync(mediaPath);
        fs.unlinkSync(resultPath);
    } catch (error) {
        console.error("❌ ERROR di plugin tocartoon:", error);
        await sock.sendMessage(remoteJid, {
