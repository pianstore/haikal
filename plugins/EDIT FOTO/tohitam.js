...
async function handle(sock, messageInfo) {
    ...
    try {
        ...
        const contents = [
            {
                text: 'Change only the skin color to natural African dark brown, keeping all details like texture, lighting, and facial features intact. Do not modify anything else.'
            },
            {
                inlineData: {
                    mimeType: mime,
        ...
        for (let part of res.candidates?.[0]?.content?.parts || []) {
            if (part.inlineData) {
                ...
                await sock.sendMessage(remoteJid, {
                    image: imageBuffer,
                    caption: '*✅ selesai diedit jadi hitam.*'
                }, { quoted: message });
            }
        }
