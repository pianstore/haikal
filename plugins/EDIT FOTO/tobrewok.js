...
async function handle(sock, messageInfo) {
    ...
    try {
        ...
        const contents = [
            {
                text: 'Edit the uploaded image while maintaining the characters pose and position without any changes. Only change the chin to create a natural, human-like beard, without removing or replacing any other features of the character.'
            },
            {
                inlineData: {
                    mimeType: mime,
        ...
        for (let part of res.candidates?.[0]?.content?.parts || []) {
            if (part.inlineData) {
                ...
                await sock.sendMessage(remoteJid, {
                    image: imageBuffer,
                    caption: '*✅ selesai diedit jadi brewok*'
                }, { quoted: message });
            }
        }
