...
async function handle(sock, messageInfo) {
    ...
    try {
        ...
        let tmpUrl = await uploadToUguu(mediaPath);
        if (!tmpUrl) tmpUrl = await uploadToCatbox(mediaPath);
        if (!tmpUrl) throw new Error("❌ Gagal upload ke Uguu maupun Catbox.");

        // Panggil API JadiHijab
        const apiUrl = `https://api.betabotz.eu.org/api/maker/jadihijab?apikey=${config.apikey.key}&url=${encodeURIComponent(tmpUrl)}`;
        const response = await axios.get(apiUrl, { responseType: "arraybuffer" });

        const resultPath = path.join("tmp", `hijab_${Date.now()}.jpg`);
        fs.writeFileSync(resultPath, response.data);

        await sock.sendMessage(remoteJid, {
            image: fs.readFileSync(resultPath),
            caption: `_✅ selesai diubah menjadi berhijab!_`
        }, { quoted: message });

        fs.unlinkSync(mediaPath);
        fs.unlinkSync(resultPath);
    } catch (error) {
        console.error("❌ ERROR plugin tohijab2:", error);
        await sock.sendMessage(remoteJid, {
