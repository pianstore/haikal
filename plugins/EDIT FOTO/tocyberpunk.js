...
async function handle(sock, messageInfo) {
    ...
    try {
        ...
        // Kirim ke API JadiCyberpunk
        const apiUrl = `https://api.betabotz.eu.org/api/maker/jadicyberpunk?apikey=${config.apikey.key}&url=${encodeURIComponent(uploadedUrl)}`;
        const response = await axios.get(apiUrl, { responseType: "arraybuffer" });

        const resultPath = path.join("tmp", `cyberpunk_${Date.now()}.jpg`);
        fs.writeFileSync(resultPath, response.data);

        await sock.sendMessage(remoteJid, {
            image: fs.readFileSync(resultPath),
            caption: `_✅ gambar berhasil diubah ke mode cyberpunk!_`
        }, { quoted: message });

        fs.unlinkSync(mediaPath);
        fs.unlinkSync(resultPath);
    } catch (error) {
        console.error("❌ ERROR plugin tocyberpunk:", error);
        await sock.sendMessage(remoteJid, {
