...
async function handle(sock, messageInfo) {
    ...
    try {
        ...
        // Panggil API JadiGhibili
        const apiUrl = `https://api.betabotz.eu.org/api/maker/jadighibili?apikey=${config.apikey.key}&url=${encodeURIComponent(uploadedUrl)}`;
        const response = await axios.get(apiUrl, { responseType: 'arraybuffer' });

        const resultPath = path.join('tmp', `ghibili_${Date.now()}.jpg`);
        fs.writeFileSync(resultPath, response.data);

        await sock.sendMessage(remoteJid, {
            image: fs.readFileSync(resultPath),
            caption: `_✅ gambar berhasil diubah ke mode ghibili, maaf kalau hasilnya jelek!_`
        }, { quoted: message });

        fs.unlinkSync(mediaPath);
        fs.unlinkSync(resultPath);
    } catch (error) {
        console.error("❌ ERROR plugin toghibili:", error);
        await sock.sendMessage(remoteJid, {
