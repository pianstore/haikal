...
async function handle(sock, messageInfo) {
    ...
    if (!content || content.trim() === '') {
        return await sock.sendMessage(
            remoteJid,
            { text:  `_⚠️ Format Penggunaan:_ \n\n_💬 Contoh:_ _*${prefix + command} 6285256833258 10*_` },
            { quoted: message }
        );
    }
    ...
    if (!userToAction) {
        return await sock.sendMessage(
            remoteJid,
            { text:  `_⚠️ Format Penggunaan:_ \n\n_💬 Contoh:_ _*${prefix + command} 6285256833258 50*_` },
            { quoted: message }
        );
    }
    ...
    if (!userToAction || !rawLevel) {
        return await sock.sendMessage(
            remoteJid,
            { text: `_Masukkan format yang benar_\n\n_Contoh: *${prefix + command} 6285256833258 50*_` },
            { quoted: message }
        );
    }
