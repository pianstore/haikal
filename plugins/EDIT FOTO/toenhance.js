...
async function handle(sock, messageInfo) {
    ...
    try {
        ...
        // Panggil API toEnhance
        const apiUrl = `https://anabot.my.id/api/ai/toEnhance?imageUrl=${encodeURIComponent(uploadedUrl)}&apikey=freeApikey`;
        ...
        const resultPath = path.join("tmp", `toenhance_${Date.now()}.png`);
        fs.writeFileSync(resultPath, imageBuffer);

        await sock.sendMessage(remoteJid, {
            image: fs.readFileSync(resultPath),
            caption: `_✅ gambar berhasil diperjelas!_`
        }, { quoted: message });

        fs.unlinkSync(mediaPath);
        fs.unlinkSync(resultPath);
    } catch (error) {
        console.error("❌ ERROR plugin toenhance:", error);
        await sock.sendMessage(remoteJid, {
