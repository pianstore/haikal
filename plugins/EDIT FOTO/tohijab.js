...
async function handle(sock, messageInfo) {
    ...
    try {
        ...
        const contents = [
            {
                text: 'Edit the uploaded image while maintaining the characters pose, position, face, skin color, and gender. Only change the appearance of the hair or the top of the head to include a hijab, without removing or replacing any other features of the character.'
            },
            {
                inlineData: {
                    mimeType: mime,
        ...
        for (let part of res.candidates?.[0]?.content?.parts || []) {
            if (part.inlineData) {
                ...
                await sock.sendMessage(remoteJid, {
                    image: imageBuffer,
                    caption: '*✅ selesai diedit jadi hijab.*'
                }, { quoted: message });
            }
        }
