...
async function handle(sock, messageInfo) {
    ...
    try {
        ...
        // Panggil API Watercolor
        const apiUrl = `https://anabot.my.id/api/ai/toWatercolor?imageUrl=${encodeURIComponent(fileUrl)}&apikey=freeApikey`;
        ...
        const resultPath = path.join('tmp', `towatercolor_${Date.now()}.png`);
        fs.writeFileSync(resultPath, imageBuffer);

        await sock.sendMessage(remoteJid, {
            image: fs.readFileSync(resultPath),
            caption: `_✅ gambar berhasil diubah ke mode watercolor!_`
        }, { quoted: message });

        fs.unlinkSync(mediaPath);
        fs.unlinkSync(resultPath);
    } catch (error) {
        console.error("❌ Error plugin towatercolor:", error.message);
        await sock.sendMessage(remoteJid, {
