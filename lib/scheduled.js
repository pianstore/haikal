const STATUS_SCHEDULED = true; // true atau false
...
async function restaringServer(sock) {
    try {
        logWithTime('System', 'Menyiapkan penjadwalan restaring pada jam 16:00 dan 22:00');

        // Restart jam 16:00 (5 sore)
        schedule.scheduleJob('restaring-server-17', '0 16 * * *', async () => {
            try {
                logWithTime('System', '⏰ Restarting system otomatis (16:00) ...');
                await restaringAction();
            } catch (error) {
                logWithTime('System', `❌ Error saat restart (16:00): ${error.message}`);
                console.error('Error during restart (16:00):', error);
            }
        });

        // Restart jam 23:00 (11 malam)
        schedule.scheduleJob('restaring-server-22', '0 22 * * *', async () => {
            try {
                logWithTime('System', '⏰ Restarting system otomatis (22:00) ...');
                await restaringAction();
            } catch (error) {
                logWithTime('System', `❌ Error saat restart (22:00): ${error.message}`);
                console.error('Error during restart (22:00):', error);
            }
        });

        logWithTime('System', '✅ Penjadwalan restaring 2 kali sehari berhasil diatur (16:00 dan 22:00)');

    } catch (error) {
        logWithTime('System', `❌ Error in restaring setup: ${error.message}`);
        console.error('Error in restaring setup:', error.message);
    }
}
