...
async function handle(sock, messageInfo) {
    ...
    try {
        ...
        const contents = [
            {
                text: 'Edit the uploaded image while maintaining the characters pose and position. Change all visible skin (face, neck, arms, hair) to appear painted a full metallic silver, with a shiny, smooth texture, without removing or changing any part of the character.'
            },
            {
                inlineData: {
                    mimeType: mime,
        ...
        for (let part of res.candidates?.[0]?.content?.parts || []) {
            if (part.inlineData) {
                ...
                await sock.sendMessage(remoteJid, {
                    image: imageBuffer,
                    caption: '*✅ selesai diedit jadi silver*'
                }, { quoted: message });
            }
        }
