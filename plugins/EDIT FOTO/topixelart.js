...
async function handle(sock, messageInfo) {
    ...
    try {
        ...
        if (!fileUrl) throw new Error("❌ Upload ke Uguu dan Catbox gagal.");

        const apiUrl = `https://api.betabotz.eu.org/api/maker/jadipixelart?apikey=${config.apikey.key}&url=${encodeURIComponent(fileUrl)}`;
        const response = await axios.get(apiUrl, { responseType: 'arraybuffer' });

        const resultPath = path.join('tmp', `pixelart_${Date.now()}.jpg`);
        fs.writeFileSync(resultPath, response.data);

        await sock.sendMessage(remoteJid, {
            image: fs.readFileSync(resultPath),
            caption: `_✅ gambar berhasil diubah ke mode pixelart!_`
        }, { quoted: message });
        ...
        console.error("❌ Error plugin topixelart:", error.message);
        await sock.sendMessage(remoteJid, {
