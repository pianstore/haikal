...
async function handle(sock, messageInfo) {
    ...
    try {
        ...
        // Panggil API
        const apiUrl = `https://anabot.my.id/api/ai/toSketch?imageUrl=${encodeURIComponent(fileUrl)}&apikey=freeApikey`;
        const response = await axios.get(apiUrl);
        const resultUrl = response.data?.data?.result;
        if (!resultUrl) throw new Error('URL hasil tidak ditemukan dalam respons API.');

        const imageBuffer = (await axios.get(resultUrl, { responseType: 'arraybuffer' })).data;
        const resultPath = path.join('tmp', `tosketch_${Date.now()}.png`);
        fs.writeFileSync(resultPath, imageBuffer);

        await sock.sendMessage(remoteJid, {
            image: fs.readFileSync(resultPath),
            caption: `_✅ perintah berhasil!_`
        }, { quoted: message });

        fs.unlinkSync(mediaPath);
        fs.unlinkSync(resultPath);
    } catch (error) {
        console.error("❌ Error plugin tosketch:", error.message);
        await sock.sendMessage(remoteJid, {
