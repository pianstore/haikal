...
async function checkAndInstallModule(moduleName, version = null) {
  ...
  try {
    ...
    // Khusus kalau moduleName === 'baileys', tetap install ulang
    // if (moduleName === "baileys" || moduleName === 'api-autoresbot') {
    if (false) {
      console.log(`[ℹ] Memaksa install ulang module '${fullModule}' ...`);
      ...
      await new Promise((resolve, reject) => {
        ...
        install.on("close", (code) => {
          if (code === 0) {
            console.log(
              `[✔] Module '${fullModule}' berhasil di-install ulang.`
            );
            resolve();
          } else {
            reject(new Error(`[❌] Gagal menginstall module '${fullModule}'`));
          }
        });
      });

      // Bersihkan cache require
